...
The purpose of this project is to build models to predict the number of social media shares that an online news article will receive. Specifically, I will train several linear regression models and random forest models, and use repeated 10-fold cross-validation to select a single champion model for each type of algorithm. This process will be repeated for every day of the week on which an article can be published (i.e., Monday, Tuesday, Wednesday, Thursday, Friday, Saturday, Sunday).
...
train_index <- createDataPartition(df$shares, p = .7)[[1]]
